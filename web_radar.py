...
with tbs[1]:
    ...
    with c1: 
        ...
        else: st.error("證交所資料抓取異常，請稍後再試。")
    with c2: 
        st.subheader("📉 上櫃排行")
        if not rk_d["TPEx"].empty: st.table(rk_d["TPEx"].iloc[:,:3])
        else: st.error("櫃買中心資料抓取異常，請稍後再試。")
...
with tbs[5]:
    ...
    st.info("""
    💡 **篩選邏輯說明：**
    1. **位階 (Price Position) < 25%**：股價處於過去半年最高與最低點區間的底部區域，確保不是追高標的。
    2. **量能爆發 > 1.8 倍**：今日成交量大於過去 5 天平均量的 1.8 倍，代表主力資金明顯進場。
    3. **流動性過濾**：自動從每日上市櫃成交金額前 250 名熱門股中進行掃描，避開流動性不佳的小型股。
    """)
    if st.button("🚀 開始大掃描", use_container_width=True):
        with st.spinner("掃描熱門標的中..."):
            rk_m = get_rk()
            cands = []
            if not rk_m["TWSE"].empty: cands += [str(x) + ".TW" for x in rk_m["TWSE"]['代號']]
            if not rk_m["TPEx"].empty: cands += [str(x) + ".TWO" for x in rk_m["TPEx"]['代號']]
            ...
            if cands:
                ...
                for tc in cands:
                    try:
                        if isinstance(raw_m.columns, pd.MultiIndex): temp_m = raw_m.xs(tc, axis=1, level=1)
                ...
                else: st.warning("目前熱門股中暫無符合條件標的。")
