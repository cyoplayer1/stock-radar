...
def analyze_stock_score(ticker, stock_name):
    ...
    try:
        ...
        if close_p > ma20: score += 20; tags.append("[站上月線]")
...
def get_rank_data(m_type):
    try:
        if m_type == "TWSE":
            ...
            fields = res['tables'][8]['fields']
            df = pd.DataFrame(raw_data, columns=fields)[['證券代號', '證券名稱', '成交金額']]
        else:
            ...
            if 'aaData' in res:
                ...
                # 上櫃通常 0:代號, 1:名稱, 這裡我們自動找數值最大的欄位作為成交額
                # 因為櫃買中心欄位常變，我們改用「成交金額」欄位特徵來抓
                df_num = df.apply(pd.to_numeric, errors='coerce').fillna(0)
                v_col = df_num.sum().idxmax() # 找出加總最大的那一欄（通常就是成交金額）
                df = df[[0, 1, v_col]]
...
SL.update({"8436.TW":"大江","8441.TW":"可寧衛","0050.TW":"台50","0056.TW":"高股息","00878.TW":"永續"})
SL.update({"00919.TW":"群益高息","00929.TW":"科技優息","00713.TW":"高息低波","006208.TW":"富邦台50"})
SL.update({"6789.TW":"采鈺","6147.TWO":"頎邦"})
FULL_STOCKS = SL

# === 5. 介面 ===
st.title("📡 股神旗艦綜合投資雷達 V5.0")
