...
with tabs[2]:
    ...
    if sid3:
        tid3 = sid3.upper()
        if "." not in tid3: tid3 += ".TW" [cite: 22]
...
with tabs[5]:
    ...
    if st.button("🚀 開始全市場大掃篩選", use_container_width=True):
        ...
        if not pool:
            ...
            if results: 
                st.dataframe(pd.DataFrame(results).sort_values('量能倍數', ascending=False), use_container_width=True) [cite: 30]
            else:
                st.warning("今日成交熱門股中，暫無符合「低檔 + 爆量」條件的標的。")
