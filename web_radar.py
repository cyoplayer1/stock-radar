...
def analyze_stock_score(ticker, stock_name):
    try:
        ...
        return {'標的名稱': f"{clean_ticker} {stock_name}", '評分': score, '收盤價': round(close_price, 2), '技術面狀態': " + ".join(tags) if tags else "空頭休息", '日K': round(k, 1), '5日均量(張)': int(avg_vol_5d / 1000), 'ticker': ticker}
    except Exception:
        return None
...
def analyze_bollinger_breakout(ticker, stock_name):
    try:
        ...
        if is_breakout:
            clean_ticker = ticker.replace('.TW', '').replace('.TWO', '')
            return {'標的名稱': f"{clean_ticker} {stock_name}", '收盤價': round(close, 2), '上軌價位': round(upper, 2), '通道寬度(%)': round(bandwidth, 1), '狀態': '🔥 突破上軌 (強勢爆發)'}
        return None
    except Exception:
        return None
...
def analyze_etf_yield(ticker, name):
    try:
        ...
        return {"ETF名稱": name, "收盤價": round(close, 2), "月線(20MA)": round(ma20, 2), "乖離率(%)": round(bias, 2), "進場判定": status}
    except Exception:
        return None
    ...
    try:
        ...
        if vol_ratio > 1.5:
            ...
            if close_today > open_today and close_today > close_yest:
                ...
                status = "⚠️ 爆量震盪 (多空交戰)"
            clean_ticker = ticker.replace('.TW', '').replace('.TWO', '')
            return {'標的名稱': f"{clean_ticker} {stock_name}", '收盤價': round(close_today, 2), '今日成交量(張)': int(vol_today / 1000), '5日均量(張)': int(vol_5ma / 1000), '爆量倍數': round(vol_ratio, 1), '異動狀態': status}
        return None
    except Exception:
        return None
...
def get_twse_top_15():
    try:
        ...
        df = pd.DataFrame(stock_data, columns=fields)[['證券代號', '
