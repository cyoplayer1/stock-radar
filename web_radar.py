...
def fetch_whale_data():
    ...
    if not data or 'data' not in data: return False, "今日資料尚未更新"
    try:
        columns = data['fields']
        df = pd.DataFrame(data['data'], columns=columns)
        t_col = [c for c in columns if '投信買賣超' in c][0]
        f_col = [c for c in columns if '外陸資買賣超' in c or '外資' in c][0]
        ...
        df_t = df[['證券代號', '證券名稱', 'T_NET']].copy(); df_t['日期'] = today
        df_f = df[['證券代號', '證券名稱', 'F_NET']].copy(); df_f['日期'] = today
        df_t.columns = ['證券代號', '證券名稱', '投信買賣超(張)', '日期']
        df_f.columns = ['證券代號', '證券名稱', '外資買賣超(張)', '日期']
...
def analyze_00981a_logic():
    ...
    try:
        ...
        df_db = pd.read_sql_query(f"SELECT * FROM trust_net_buy WHERE 證券代號 IN ({p})", conn, params=list(target_stocks.keys()))
        conn.close()
        
        results = []
        for sid, group in df_db.groupby('證券代號'):
            ...
            if w >= (limit - 0.5): hint, icon = "❌ 嚴禁追高", "🛑"
            elif w >= (limit - 2.0): hint, icon = "🟡 子彈將盡", "⚠️"
            else: hint, icon = "🟢 空間充裕", "✅"

            results.append({
                "代號": sid, "股票名稱": target_stocks[sid][0], "權重狀態": f"{icon} {w}%", "跟單建議": hint,
...
if page == "🕵️‍♂️ 00981A 經理人跟單":
    ...
    if st.button("🚀 全自動即時掃描", use_container_width=True):
        ...
        res = []
        with ThreadPoolExecutor(max_workers=10) as ex:
            futs = [ex.submit(analyze_star_score, t, inst_map, hot_list) for t in s_list]
            for i, f in enumerate(as_completed(futs)):
                if f.result(): 
                    item = f.result(); item['標的'] = s_list[i]
                    res.append(item)
        if res:
            df = pd.DataFrame(res).sort_values(by="星星數", ascending=False)
            df.reset_index(drop=True, inplace=True); df.insert(0, '排名', df.index + 1)
            st.dataframe(df[['排名', '標的', '星等', '現價', '條件']], use_container_width=True, hide_index=True)
    ...
    if os.path.exists("whale_tracker.db"):
        conn = sqlite3.connect("whale_tracker.db")
        table = "trust_net_buy" if whale == "trust" else "foreign_net_buy"
        ...
        for sid, group in df.groupby('證券代號'):
            ...
            if buy_days >= 1:
                hot.append({"名稱": group.iloc[-1]['證券名稱'], "連買天數": buy_days, "累積買超": int(sum(diffs[-buy_days:]))})
