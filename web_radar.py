...
def analyze_stock_score(ticker, stock_name):
    ...
    try:
        ...
        if v_5d < 1000000: return None # 1000張門檻
        score, tags = 0, []
...
SL = {
    ...
    "00919.TW": "群益高息", "00929.TW": "復華科技", "00713.TW": "高息低波",
    "006208.TW": "富邦台50", "6789.TW": "采鈺", "6147.TWO": "頎邦"
}

# === 5. 網頁介面 ===
st.title("📡 股神系統雷達 - 旗艦整合版")
...
with tabs[2]:
    ...
    if sid:
        ...
        if not d.empty:
            ...
            tr_k = go.Candlestick(x=d.index, open=d['Open'], high=d['High'], 
                                  low=d['Low'], close=d['Close'], name='K線')
            fig.add_trace(tr_k, row=1, col=1)
    ...
    if st.button("啟動波段突破掃描"):
        ...
        for t, n in SL.items():
            ...
            if df['Close'].iloc[-1] > df['UP'].iloc[-1]:
                brs.append({'標的':f"{t} {n}",'收盤':round(df['Close'].iloc[-1],2),'狀態':'🔥突破'})
