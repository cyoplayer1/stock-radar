...
# === 2. 核心清洗與計算函數 ===
def clean_it(df):
    """專門對付 Python 3.14 與 yfinance 新版格式的數據清洗器"""
    ...
    if st.button("🚀 啟動掃描", use_container_width=True):
        ...
        for t, name in SL.items():
            try:
                ...
                if close > ma20: sc += 20; tags.append("[站上月線]")
                dkd = calculate_kd(df)
                if dkd['K'].iloc[-1] > dkd['D'].iloc[-1]:
                    if dkd['K'].iloc[-2] <= dkd['D'].iloc[-2]: sc += 40; tags.append("[日金叉]")
                    else: sc += 20; tags.append("[日偏多]")
                rl.append({'標的': f"{t.split('.')[0]} {name}", '評分': f"{sc}分", '收盤': round(float(close), 2), '狀態': " + ".join(tags) if tags else "休息", 'Sort': sc})
            except: continue
