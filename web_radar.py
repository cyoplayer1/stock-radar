...
def check_low_breakout(ticker, name):
    try:
        ...
        if pos < 0.25:
            return {'標的': f"{ticker.split('.')[0]} {name}", '收盤價': round(curr_p, 2), '量能倍數': round(v_now/v_avg, 2), '低檔位置': f"{round(pos*100, 1)}%", '今日張數': int(v_now/1000)}
    except: return None
    return None
...
# === 6. 介面設計 ===
st.title("📡 股神系統旗艦整合版")
t1, t2, t3, t4, t5, t6 = st.tabs(["🎯 股神雷達", "💰 成交排行", "📈 互動看盤", "🚀 波段掃描", "🔥 量能監控", "🔍 全市場低檔快篩"])
