...
with tbs[0]:
    if st.button("🚀 啟動掃描", use_container_width=True):
        ...
        for t, name in SL.items():
            try:
                # 兼容處理單層與多層索引
                if isinstance(data_raw.columns, pd.MultiIndex): temp_df = data_raw.xs(t, axis=1, level=1)
                else: temp_df = data_raw[t]
                df = calculate_kd(temp_df)
                ...
                if df['K'].iloc[-1] > df['D'].iloc[-1]:
                    if df['K'].iloc[-2] <= df['D'].iloc[-2]: sc += 40; tags.append("[日金叉]")
